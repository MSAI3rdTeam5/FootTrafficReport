    ...
    chat_prompt = [
        {
            ...
            5. **시간대별 유동인구 분석**
             ...
             6. **결론 및 사업 전략 제안**  
                - 종합적으로 분석된 유동인구 데이터를 바탕으로 사업 적합성이 높은 업종을 제안  
                - 특정한 위치나 상권에서 실행 가능한 **사업 모델 및 실행 계획** 제공
            ...
            2. 보고서에 **제목**은 페르소나를 기반으로 설정해주세요..
            3. **각 항목별 주요 발견을 데이터 기반으로 분석**하고, 설명하세요.
            4. **추천 전략**은 전략(입지, 운영시간, 마케팅, 서비스 차별화 등)을 구체적으로 250자 이상 작성해주세요.
            5. **결론에서는 사장님이 실행할 수 있는 구체적인 액션 플랜**을 제시하세요.
            6. 보고서가 나올때 html언어로 나오게 해주세요. 보고서를 css로 디자인해주세요.    
            ...
            4. **추천전략:**과 **주요 발견:** 이 두개는 보고서와 같이 bold체를 고정해주세요.
...
def gpt_response_v2(persona, user_input, data, start_date, end_date):
    #데이터 시각화 그래프 경로 불러오기
    graph_paths = create_visualizations(data, start_date=start_date, end_date=end_date)
    ...
    chat_prompt = [
        {
            ...
            4. **추천 전략**은 창업 아이템과 연관된 전략(입지, 운영시간, 마케팅, 서비스 차별화 등)을 구체적으로 250자 이상 작성해주세요.
