...
class PersonTracker:
    ...
    async def detect_and_track(self, source, cctv_id):
        ...
        skip_interval = max(1, total_frames // 200)
        ...
        for result in results:
            ...
            for box, kpts in zip(boxes, keypoints_data):
                ...
                if face_area:
                    frame = self.apply_face_blur(display_frame, face_area) #바운딩 박스 없이 블러 streaming 처리 : display_frame -> frame
            ...
            out.write(frame)
            cv2.imshow("Face Blurred Person Tracking", display_frame) #display_frame -> frame
